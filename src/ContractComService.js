...
export default class ContractComService {
  constructor(store) {
    ...
    static processCommonErrors(error) {
      ...
      if (error === 'Invalid status 6985') { return Promise.reject('You rejected transaction on your Nano Ledger.'); }

      // transaction rejected when using nano ledger
      if (error === 'Invalid status 6a80') { return Promise.reject('Please enable contract data on your Nano Ledger.'); }

      // transaction rejected when using metamask
      if (error.message !== undefined
            && error.message.startsWith('Error: MetaMask Tx Signature: User denied')) { return Promise.reject('You rejected transaction using metamask.'); }
