...
export default ({ employee, ESOPState, signHandler, showPaperCopyHandler }) => {
  ...
  /* If you want to use HTML elements you need to wrap them in div.
  Reason is you are writing code in JSX which is later transpiled into JS.
  ...
  const table = [
    ...
    },
    {
      ...
      value: ContractUtils.getEmployeeStateName(employee.state, employee.suspendedAt,
        employee.timeToSign <= ESOPState.currentBlockTimestamp),
    },
  ];

  const cellClickHandler = (rowNumber) => {
    const element = document.getElementById(`descSign${rowNumber}`);
    if (element !== null) {
    ...
    <div className="employee_sign_esop">
          ...
          <div className="esop_title">
            <h3>Fifth force GmBH identifed by address {ESOPState.companyAddress}
              <a
                className="inline_link"
                target="_blank"
                href={ContractUtils.formatEtherscanUrl(ESOPState.companyAddress,
                  ESOPState.networkId)}
              >
      ...
      <div className="row">
        <p className="col-xs-12">
          I hereby subscribe for the Issued Options for shares in {Config.ipfs_tags.company} under
          the terms and conditions as set out in the ESOP Smart Contract at address
          {ESOPState.RoTAddress} and made available to me in {Texting.termsDocumentTitle}
        </p>
      </div>
      <div className="row">
        <div className="col-xs-12">
